  ...
  <div class="order-radio" :class="classes">
    <div class="flex flex-wrap sm:gap-x-[20px] order-radio-top sm:gap-y-[17px] items-baseline md:gap-[28px] gap-[21px]">
      <div :data-id="option.id" ref="root" class="order-option flex flex-col" v-for="(option, i) in props.options">
        <input class="hidden cursor-pointer" :id="props.name + option.id" type="radio" :checked="handleChecked(option, i)" :name="props.name" v-model="radioValue" :value="option" @change="handleChange(option)" />
        <label :for="props.name + option.id" class="order-option-container cursor-pointer">
          <div :class="{ 'bg-[#1B1B1B]  hover:bg-[#262626]': props.bg }" class="order-option-card justify-center overflow-hidden flex sm:mb-[13px] mb-[16px] rounded-[13px] sm:w-[100px] sm:h-[100px] w-[104px] h-[104px]">
