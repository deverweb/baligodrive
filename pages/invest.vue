<template>
  <div class="w-full overflow-x-hidden">
    <section class="invest-offer overflow-hidden w-full bg-[#202020] px-[80px] sm:px-[25px] md:min-h-0 lg:px-[50px] lg:h-auto flex xl:min-h-800 min-h-[900px] h-screen rounded-t-[70px] sm:rounded-t-[44px]">
      <div class="flex flex-col w-full lg:pb-[196px] sm:pb-[48px] md:pb-[324px] h-full relative justify-center xl:justify-start pt-[86px] sm:pt-[64px] xl:pt-[166px]">
        <h1
          v-html="test('ИнвестиЦИи<br />в бизнес аренды скутеров на балИ', 'Investing<br />in a scooter rental business in Bali')"
        ...
        ></h1>
        <p class="mb-[80px] max-w-[570px] sm:w-full sm:max-w-full sm:text-[16px] sm:leading-[20px] sm:mb-[40px] lg:mb-[61px] pl-[5px] lg:pl-0 leading-[1.25] font-Helvmed text-[20px]">
          {{ test("От 24% годовых в долларах. Ежемесячные выплаты. Работает, как депозит в банке.", "From 24% per annum in dollars. Monthly payments. Works like a bank deposit.") }}
        </p>
        <div
        ...
        <div class="flex items-center sm:items-center lg:flex-col lg:items-start sm:gap-[28px] lg:gap-[47px] lg:pl-0 pl-[4px]">
          <TheButton @click="handleInvestScroll" class="w-[330px] sm:max-w-[340px] sm:w-full sm:h-[70px] lg:w-[300px] lg:h-[82px] lg:mr-0 mr-[50px] gap-[10px] h-[79px] btn-primary__dark">
          ...
          <button @click="handleCalcScroll" class="flex items-center sm:ml-0 lg:ml-[27px]">
            ...
            <span class="border-b-[1px] pb-[0px] leading-[1.2] border-light border-solid text-[#fff] text-[18px]">{{ test("Калькулятор доходности", "Yield Calculator") }}</span>
          </button>
        </div>
      </div>
    </section>
    <section class="invest-blocks sm:mb-[-130px] md:mb-[-110px] sm:pt-[75px] mb-[-80px] relative bg-[#181818] md:pt-[80px] pt-[134px] pb-[130px]">
      <h2 class="font-Euroblack sm:max-w-[80%] sm:mx-auto uppercase text-[58px] sm:text-[32px] sm:leading-[31px] md:mb-[55px] md:text-[46px] tracking-[-0.6px] md:tracking-[-1.3px] text-light text-center mb-[69px] sm:mb-[45px]">
        {{ test("Почему байки?", "Why bikes?") }}
      </h2>
      <SectionCustomCards class="mb-[80px]" :blocks="locale == 'ru' ? ruSixBlocks : engSixBlocks"></SectionCustomCards>
    </section>
    <section class="tabled mb-[-70px] sm:rounded-b-none md:rounded-b-[70px] relative sm:pb-[131px] md:pb-[70px] pb-[120px] sm:pt-[75px] md:pt-[95px] pt-[121px] bg-light text-dark rounded-t-[70px] md:rounded-t-[44px]">
      <div class="container">
        <h2 class="font-Euroblack text-[58px] lil:text-[22px] sm:leading-[30px] sm:text-[27px] md:text-[46px] uppercase md:tracking-[-1.3px] leading-[0.96] tracking-[-0.6px] xl:text-center text-left md:mb-[-56px] mb-[-49px]">
          {{ test("ВЫГОДНЕЕ ЧЕМ НЕДВИЖИМОСТЬ", "BETTER THAN REAL ESTATE") }}
        </h2>
        <div class="tabled-main relative">
          <div class="catalog-drag xl:flex hidden justify-end xl:max-h-[35px] xl:absolute xl:left-[30px] xl:top-[133px] sm:top-[70px] sm:left-[10px]">
            <SvgDragAnim class="h-[43px] w-[105px] sm:h-[30px] sm:w-[70px]"></SvgDragAnim>
          </div>
          <div class="tabled-col-desc md:pb-[55px] pb-[150px] sm:pt-[130px] pt-[200px] xl:overflow-x-scroll xl:overflow-y-hidden">
            <div class="tabled-row-head">
              <div class=""></div>
              <div class="relative" v-html="test(`Инвестиции в &nbsp;<br />аренду байков`, `Investment in &nbsp;<br />bike rental`)"></div>
            ...
            <div class="tabled-row">
              <div v-html="test('Срок возврата инвестиций<br />(тело депозита)', 'Return period of investment<br />(deposit body)')"></div>
            ...
            <div class="tabled-row">
              ...
              <div>
                <ul>
                  <li>1. {{ test("Долгий срок ожидания первой прибыли", "Long waiting period for the first profit") }}</li>
                  <li>2. {{ test("Большой порог входа", "Large entry threshold") }}</li>
                  <li>3. {{ test("Заморозка денежных средств на длительный период", "Freezing funds for a long period") }}</li>
                </ul>
              </div>
            </div>
            <div class="tabled-row">
              ...
              <div>
                {{ test("Риски для инвестора отсутствуют, т.к. компания все риски берёт на себя.", "There are no risks for the investor, because The company assumes all risks.") }}
              </div>
              <div>
                <ul>
                  ...
                  <li>
                    2.
                    {{ test("Юридические риски (оформленные земли и объекта недвижимости)", "Legal risks (registered land and real estate)") }}
                  </li>
                </ul>
              </div>
            </div>
            <div class="tabled-row">
              ...
              <div>
                {{ test("Подходит для тех, кто располагает большими средствами и инвестирует на долгосрок", "Suitable for those who have large funds and invest for the long term") }}
              </div>
            </div>
          </div>
        </div>
    ...
    <section class="calc pb-[130px] sm:rounded-t-[44px] sm:bg-[#181818] md:pb-[100px] md:rounded-t-none md:bg-transparent md:relative sm:pt-[60px] md:pt-[174px] pt-[123px] relative rounded-t-[70px] bg-[#181818]">
      <div class="container">
        <div class="uppercase font-Euroblack text-[58px] md:text-[48px] leading-[0.96] tracking-[-0.6px] text-center mb-[26px] lil:text-[20px] sm:text-[24px]">
        ...
        <div class="section-desc text-center mb-[72px] md:mb-[21px] max-w-[375px] mx-auto">
          {{ test("Рассчитайте доходность своих инвестиций в бизнес аренды байков на Бали.", "Calculate the return on your investment in a bike rental business in Bali.") }}
        </div>
        <div class="calc-main flex md:flex-col justify-between">
          <div class="calc-left sm:pt-[24px] pt-[32px] md:w-full lg:w-[38%] sm:px-0 md:px-[38px] w-[45.5%]">
            <div class="calc-input md:mb-[72px] sm:mb-[54px] mb-[87px]">
              <div class="calc-input-label sm:text-[14px] lil:text-[12px] flex items-baseline justify-between font-Euroblack uppercase text-[20px] sm:mb-[18px] mb-[37px]">
            ...
            <div class="calc-slider sm:mb-[90px] md:mb-[105px]">
              <div class="calc-slider-label items-center lil:text-[12px] sm:mb-[22px] sm:text-[14px] tracking-[-0.6px] flex justify-between font-Euroblack uppercase text-[20px] mb-[30px]">
                <span>{{ test("СРОК ИНВЕСТИРОВАНИЯ", "INVESTMENT TERM") }} </span>
                <span>{{ test("МЕС.", "MONTHS") }} </span>
              </div>
              <div class="calc-slider-tumb">
                <div class="calc-slider-nums sm:text-[14px] flex justify-between align-baseline text-light opacity-20 text-[16px] tracking-[-0.6px] font-Euroblack uppercase mb-[14px]">
                  <span>24</span>
                  <span>36</span>
                </div>
                <ClientOnly>
    ...
    <section class="invest-blocks sm:bg-transparent sm:pb-[100px] md:pb-[100px] md:mb-[-80px] relative sm:pt-[66px] md:pt-[89px] pt-[120px] pb-[50px] md:rounded-t-[44px] md:bg-dark-300 md:rounded-b-none rounded-b-[70px] sm:rounded-b-[44px]">
      <h2
      ...
      <p class="section-desc text-center lil:max-w-[90%] max-w-[405px] mx-auto text-light mb-[52px]">
        {{ test("Стать инвестором компании Bali.Go можно всего за несколько простых шагов", "You can become an investor in Bali.Go in just a few simple steps") }}
      </p>
      <SectionCustomCards class="mb-[80px] bg-transparent" :blocks="test(ruProcessBlocks, engProcessBlocks)"></SectionCustomCards>
    </section>
    <section class="bg-light md:relative sm:pt-[76px] sm:pb-[178px] md:pb-[202px] invest-form-section text-dark rounded-t-[70px] md:pt-[91px] pt-[122px] md:rounded-t-[44px] pb-[280px]">
      <div class="container">
        <div class="invest-form-title sm:mb-[18px] lil:text-[22px] sm:tracking-[-0.9px] sm:text-[27px] md:w-full md:text-[44px] w-[90%] mx-auto mb-[28px] tracking-[-1.6px] font-Euroblack uppercase leading-[0.96] text-center text-[52px]">
          {{ test("ИНВЕСТИРУЙТЕ В Бизнес аренды скутеров НА БАЛИ", "INVEST IN BALI Scooter Rental Business") }}
        </div>
        <div class="invest-form-subtitle md:max-w-[470px] md:mx-auto sm:mb-[28px] section-desc md:mb-[49px] text-center mb-[60px]">
          {{ test("Заполните форму и наш менеджер свяжется с вами в ближайшее время для обсуждения деталей.", "Fill out the form and our manager will contact you shortly to discuss the details.") }}
        </div>
        <Form @submit="onSubmit" class="flex-wrap invest-form-tag flex">
          <SectionCustomTextField type="string" class="ci__last-form w-[360px] max-w-full md:h-[63px]" :name="'client_name'" :placeholder="test('Имя', 'Name')">
            <SvgPersonIcon class="sm:w-[12] sm:h-[14px]" fill="#000" opacity="1"></SvgPersonIcon>
          </SectionCustomTextField>
          <SectionCustomPhoneField class="ci__last-form w-[360px] max-w-full md:h-[63px]" type="invest" name="client_phone">
          ...
          <SectionCustomSelectField class="w-[360px] h-[67px] md:h-[63px] max-w-full cs__invest-form" name="messenger" styleType="invest" :defaultLabel="test('Мессенджер', 'Messenger')" :options="messengerOptions">
